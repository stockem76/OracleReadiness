...
Generate the JPK_ST_KR asset register section using the revised JPK_CIT audit file format for Poland. This report includes asset information from both accounting and tax asset books for the same asset records and follows the XML schema defined by the Polish tax authorities.

The JPK_ST_KR report is submitted annually along with the JPK Accounting report under JPK_CIT. Use the existing scheduled process for JPK Accounting to generate the report by specifying values for these parameters:

• Corporate Asset Book
• Accounting Asset Book
• Tax Asset Book

All three parameters are required to generate the JPK_ST_KR report. If asset book information isn’t provided, the process generates the JPK_PD Accounting file instead.
...
• Ensures compliance with Poland's JPK_CIT statutory reporting requirements by enabling submission of asset register data in the required JPK_ST_KR format.
...
• Maintain separate asset books for accounting and tax reporting as required by JPK_ST_KR.
...
• Record the retirement reason during each retirement transaction starting January 1, 2026.
   ...
   These values are used during JPK_ST_KR report generation in 2027 to populate mandatory reporting fields.
