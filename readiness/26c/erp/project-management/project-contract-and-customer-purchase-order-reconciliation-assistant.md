...
# Project Contract and Customer Purchase Order Reconciliation Assistant

`⚙️ Steps to Enable` · `💡 Tips & Considerations` · `🔐 Access Requirements`

> 📖 [View on Oracle Help Center](https://docs.oracle.com/en/cloud/saas/readiness/erp/26c/ppm26c/26C-ppm-wn-f49414.htm)

Reconcile project contracts with attached customer purchase order documents by comparing key attributes and identifying mismatches so corrective action can be taken. This automated reconciliation helps reduce contract approval cycles, accelerate the contract funding process, and achieve faster invoicing and revenue recognition.

The assistant performs the following reconciliations between project contracts and attached customer purchase order documents to ensure that:

a) The **purchase order number** on the purchase order document attached to the contract matches the purchase order number specified on the contract or contract lines.

b) The **currency**specified on the purchase order document matches the currency defined on the contract.

c) The **customer’s****name**on the purchase order document matches the customer name on the contract.

d) The **bill-to and ship-to addresses** on the purchase order document must match the corresponding details on the contract parties.
...
• The assistant reconciles the contract with the attached purchase order document. Two issues are identified, and the assistant provides the user with details regarding the cause and suggests a resolution.
...
• Contract name {Contract Name}
• Validate contract {Contract Number}
• The assistant reconciles the contract with the attached purchase order document. No issues are identified.
...
• The Project Contract and Customer Purchase Order Reconciliation Assistant is a preconfigured template. You need to create your own agent using the preconfigured template. To learn how to set up AI agents, see Create AI Agents Using Preconfigured Agent Team Templates.
...
• If a purchase order document is not attached to the contract, the assistant informs the user that reconciliation cannot be performed because the required document(s) are missing.

## 🔐 Access Requirements

To access the **Project Contract and Customer Purchase Order Reconciliation Assistant**, users must be assigned the predefined **Customer Contract Administrator** or **Customer Contract Manager** role, or a custom role copied from these predefined roles.
